...
exports.fillDemoAccount = function(user, callback)
{
	return new sequelize.Promise(function(resolve, reject) {
		...
		t_to_add_1.push({amount: initial_1_amount, description: initial_1_description, datetime: now - 70*24*60*60});
		for (var i = 29; i > 0; i--)
		{
			...
			t_to_add_1.push({amount: -amount, description: description, datetime: datetime});

			if (total_1 < 300)
			{
				var amount = rand(100, 1000);
				t_to_add_1.push({amount: amount, description: initial_1_description, datetime: datetime+rand(10,1000)});
				total_1 = total_1 + amount;
			}
		}
		...
		t_to_add_2.push({amount: initial_2_amount, description: initial_2_description, datetime: now - 50*24*60*60});
		for (var i = 19; i > 0; i--)
		{
			...
			t_to_add_2.push({amount: -amount, description: description, datetime: datetime});

			if (total_2 < 1000)
			{
				var amount = rand(1, 10)*100;
				t_to_add_2.push({amount: amount, description: initial_2_description, datetime: datetime+rand(10,1000)});
				total_2 = total_2 + amount;
			}
		}

	    sequelize.Promise.map( [wallet_1_name, wallet_2_name], function(name) {
	      return user.createWallet({ name: name, currency: currency });
	    }).spread(function(wallet_1, wallet_2) {

	    	/// @todo: do with raw query. Too slow now.
	    	sequelize.Promise.map( t_to_add_1, function(data){
	    		return wallet_1.insertTransaction({ description: data.description, amount: data.amount, datetime: data.datetime });
	    	}).then(function(){
	    		sequelize.Promise.map( t_to_add_2, function(data){
		    		return wallet_2.insertTransaction({ description: data.description, amount: data.amount, datetime: data.datetime });
		    	}).then(function(){
		    		resolve();
		    	});
	    	});
